...
namespace DesignationOfRisers.Models
{
    public partial class PipingSystemMdl : ObservableObject
    {
        ...
        public long? SelectedMarkId
        {
            ...
            get => SelectedMark?.Id.IntegerValue;
            set
            {
                if (value.HasValue)
                {
                    SelectedMark = Marks.FirstOrDefault(mark => mark.Id.IntegerValue == value.Value);
                }
                else
                {
                    SelectedMark = null;
                }
                OnPropertyChanged(nameof(SelectedMarkId));
            }
#else
        ...
        public static PipingSystemMdl FromSerializable(PipingSystemMdlSerializable serializable, Document document)
        {
            ...
            var marks = serializable.MarkIds
                           .Select(id => document.GetElement(new ElementId(int.Parse(id))))
                           .ToList();

            var selectedMark = serializable.SelectedMarkId != null
                                ? document.GetElement(new ElementId(int.Parse(serializable.SelectedMarkId)))
                                : null;
#else
            var marks = serializable.MarkIds
